		...
		<div class="container">
			<div class="row">
				<div class="col-md-12 col-12">
					<h2 class="breadcrumb-title mb-2">Recyclable Listings</h2>
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb justify-content-center mb-0">
							<li class="breadcrumb-item"><a href="{{url('/')}}"><i class="ti ti-home-2"></i></a></li>
							<li class="breadcrumb-item active" aria-current="page">Recyclable Listings</li>
						</ol>
					</nav>
				</div>
			</div>
									...
									<div class="accordion border-bottom mb-3">
										<div class="accordion-header" id="accordion-headingFive">
											<div class="accordion-button p-0 mb-3" data-bs-toggle="collapse" data-bs-target="#accordion-collapseSix" aria-expanded="true" aria-controls="accordion-collapseSix" role="button">
												Enter pincode for search

											</div>
										</div>
										<div id="accordion-collapseSix" class="accordion-collapse collapse show" aria-labelledby="accordion-headingFive">
											<div class="mb-3">
												<div class="position-relative">
                                                    <input type="text" class="form-control" id="locationInput" placeholder="Enter Pincode" name="pincode">
                              ...
                              <div class="service-list">
                                <div class="service-cont">
                                  ...
                                  <div class="service-cont-info">
                                    <span class="badge bg-light fs-14 mb-2">{{ $post->resource->resource_name }}</span>
                                    <h3 class="title">
                                      <a href="{{ url('recyclable_listing_details/'.$post->id) }}">{{ $post->resource_name }}</a>
                                    </h3>
                                   ...
                                   <span>
                                        ...
                                        @for ($i = 1; $i <= 5; $i++)
                                            ...
                                            @else
                                                <!--<i class="far fa-star" style="color: #ffc107;"></i> {{-- Outlined gold star --}}-->
                                                 <i class="fas fa-star" style="color: #ddd;"></i> {{-- Filled gray star --}}
                                            @endif
                                        @endfor
                                    </span>
                                  ...
                                  <a href="#" data-id="{{ $post->id }}" data-bs-toggle="modal" data-bs-target="#enquiryModal" class="btn btn-light connect-listing mobcon">Connect</a>
                                    ...
                                    <div class="  d-flex justify-content-between gap-4">
                                        <div class="social-icon">
                                            ...
                                            <span class="wp-dis-own"> Share on:   <a href="https://wa.me/?text={{ urlencode('This post from The ZeroWaste Community Tool might interest you, check it out : ' . url('recyclable_listing_details/'.$post->id)) }}" target="_blank" class="me-2">
                                                    <img src="{{asset('frontend/assets/img/icons/whatsapp.svg') }}" class="img" alt="WhatsApp">
                                                </a></span>
                                            @else
                                                <a href="{{ route('consumer_login', ['redirect_wp' => url('recyclable_listing_details/' . $post->id)]) }}" target="_blank">
                                                    <img src="{{asset('frontend/assets/img/icons/whatsapp.svg') }}" class="img" alt="WhatsApp">
                                                </a>
                                            @endif
                                        </div>
 ...
 <nav aria-label="Page navigation">
  <ul class="paginations d-flex justify-content-center align-items-center">
    ...
    @foreach ($posts->getUrlRange(1, $posts->lastPage()) as $page => $url)
      <li class="page-item ms-4 ms-lg-0 me-2">
    ...
    @endif
                ...
                <div class="row mt-5 text-center">
                     <div class="col-md-3">
                                                                           ...
                                                                           <h5> Have something that can be recycled?  </h5>
                                                                    </div>
                                                                    <div class="col-md-3 mt-2 mt-md-0 mb-2">
                                                                       <a href="{{route('recyclable_add_post')}}" class="btn btn-linear-primary btn-lg w-100">
                                                                            Add Your Listing</a>
                                                                    </div>
                                                                    <div class="col-md-3 mb-2">
                                                                        <a href="{{url('/')}}" class="btn btn-linear-primary btn-lg w-100">  Go back to Home Page</a>
                                                                    </div>
...
<div class="modal fade" id="enquiryModal" tabindex="-1" aria-labelledby="enquiryModalLabel" aria-hidden="true">
  ...
  <button type="button" class="btn-close position-absolute" style="right: 1rem;" data-bs-dismiss="modal" aria-label="Close"></button>
      ...
      <div class="modal-body p-0">
        <form action="{{ route('recyclable_enquiry_save') }}" method="POST">
          @csrf
          ...
          <button type="submit" class="btn btn-linear-primary btn-lg   w-md-50  ">Send Message</button>
          </div>
        </form>
      </div>
...
$(document).ready(function() {
    $('#filterForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('recyclable_post_filter') }}",
            method: "GET",
            data: $(this).serialize(),
            success: function(response) {
                ...
                if (!posts || posts.length === 0) {
                    ...
                    posts.forEach(function(post) {
                        ...
                        let resourceName = post.resource ? post.resource.resource_name : 'N/A';
                        ...
                        let shareLink;
                        if (userId) {
                            shareLink = `<a href="https://wa.me/?text=${encodeURIComponent('This post from The ZeroWaste Community Tool might interest you, check it out : ' + basePath + '/recyclable_listing_details/' + post.id)}" target="_blank">
                                            <img src="{{ asset('frontend/assets/img/icons/whatsapp.svg') }}" class="img" alt="WhatsApp">
                                         </a>`;
                        } else {
                            shareLink = `<a href="/${projectFolder}/consumer_login?redirect_wp=${basePath}/recyclable_listing_details/${post.id}" target="_blank">
                                            <img src="{{ asset('frontend/assets/img/icons/whatsapp.svg') }}" class="img" alt="WhatsApp">
                                         </a>`;
                        }
                        ...
                        $('#listings-container').append(`
                            <div class="service-list">
                                <div class="service-cont">
                                    <div class="service-cont-img">
                                        <a href="recyclable_listing_details/${post.id}">
                                            <img src="${imageUrl}" alt="Listing Image" class="img-fluid serv-img">
                                        </a>
                                    </div>
                                    <div class="service-cont-info">
        ...
        <div class="service-action">
                                     <span class="wp-dis-own">Distance: ${post.distance || 0}km</span>
...
<script>
  ...
  function applySort(sortBy) {
    ...
    $.ajax({
      url: "{{ route('recyclable_post_sort') }}", // Separate route for sorting
      method: "GET",
      data: { sort_by: sortBy },
      success: function (response) {
  ...
  function displayResults(posts) {
    ...
    posts.forEach(function (post) {
      let imageUrl = post.image_url;
      let resourceName = post?.resource?.resource_name || 'N/A';
        let userName = post.user ? post.user.name : 'N/A';
      let rating = post?.rating || 'N/A';
      let userImage = "{{ asset('frontend/assets/img/user.png') }}";
      let address = post?.address || 'Unknown Address';
      let price = post?.resource_price ? `₹${post.resource_price}` : 'N/A';
      let weight = post?.weight
        ? `${post.weight.min_weight} ${post.weight.min_measure} - ${post.weight.max_weight} ${post.weight.max_measure}`
        : 'N/A';
     let baseUrl = window.location.origin;
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.connect-listing', function(e) {
            ...
            $.ajax({
                url: "{{ url('/get_recyclable_post_details') }}",
                method: 'GET',
                data: { dataId: dataId },
                success: function(response) {
...
$(document).ready(function() {
  // Fetch allowed pincodes from the database
  $.ajax({
    url: "{{url('/api/pincodes') }}",
    method: 'GET',
    success: function(data) {
      allowedPincodes = data;
      console.log('Allowed Pincodes:', allowedPincodes);
    }
  });
  ...
  $('#locationInput').on('input', function() {
    ...
    const matchedPincodes = allowedPincodes.filter(pincode =>
  pincode.toString().startsWith(inputVal)
);


    $('#suggestions').empty();
    if (matchedPincodes.length > 0) {
      matchedPincodes.forEach(function(pincode) {
        $('#suggestions').append(`<li class="list-group-item suggestion-item">${pincode}</li>`);
      });
    } else {
      $('#suggestions').append('<li class="list-group-item">No matching pincodes</li>');
    }
  });
