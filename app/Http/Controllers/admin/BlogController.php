...
class BlogController extends Controller
{
    public function blog_category_list(){
        // Get the authenticated user's type from the default guard
    $authType = auth()->user()->type;

    // Get the authenticated user's type from the volunteer guard
    $volunteerAuthType = auth('volunteer')->check() ? auth('volunteer')->user()->type : null;
    
        $blog_category = BlogCategory::orderBy('id', 'DESC')->get();
        // echo'<pre>';
        // print_r($state);
        // die;
        $data = compact('blog_category','authType','volunteerAuthType');
    ...
    if($authType){
        ...
        $url = route('blog.volunteer_blog_category_save'); 
    }
 ...
 public function blog_tag_delete($id)
{
 ...
 $slug = Str::slug($request->blog_name, '-'); 
        ...
        Alert::success('success','Tag Deleted Successfully');
        return redirect()->route('admin_blog_list');
    }
    public function blog_edit($id)
    {
         $url = route('admin_blog_update', $id);
    ...
    public function blog_view($id){
        ...
        // Get the categories and tags already associated with this blog
        $categories = BlogCategory::whereIn('id', explode(',', $blog->category))->pluck('category_name'); // Replace 'name' with the actual column name for the category name
    $tags = BlogTag::whereIn('id', explode(',', $blog->tag))->pluck('tag_name'); // Replace 'name' with the actual column name for the tag name
    // Check posted_by: either 'admin' or volunteer ID
    $posted_by_name = '';
    ...
    if ($blog->posted_by === 'admin') {
        ...
        if ($volunteer) {
            ...
            $posted_by_name = $volunteer->name; // Adjust field if needed
            
        }
        
    }

        return view('admin.blog.blog_view',compact('blog','category', 'tag', 'categories', 'tags', 'posted_by_name')); 
    }
