...
$sabPosts = SABPost::join('s_a_b_resource_posts', 's_a_b_posts.id', '=', 's_a_b_resource_posts.post_id')
 ->select('s_a_b_posts.*', 's_a_b_resource_posts.resource_type')
    ->where('s_a_b_posts.pincode', $pincodeValue)
     ->where('s_a_b_posts.user_id', '!=', $userId)
      ->where('s_a_b_posts.active', '=', 1)
    ->where('s_a_b_resource_posts.resource_type',$resource)
      ->where('s_a_b_posts.id','=',$post_id)
    ->first();
    if ($sabPosts) {
     Log::info('SAB Post Found: ' . json_encode($sabPosts));
    // Proceed to send email with this post
    self::sendEmail($userEmail, $details, $sabPosts, $sabPosts->id, $notifyId, $sabPosts->user->user_type);
} else {
    Log::info("No SAB post found with the given conditions.");
}
